...
class Connection(QGraphicsLineItem):
    def __init__(self, start_connector, end_connector=None):
        ...
        self.setAcceptHoverEvents(True) # Разрешаем обработку событий мыши
        self.setFlag(QGraphicsLineItem.ItemIsSelectable) # Делаем элемент выбираемым
    ...
    # Добавляем метод для визуального выделения
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.RightButton: # Проверяем правую кнопку мыши
            self.delete_connection() # Вызываем метод удаления
        else:
            super().mousePressEvent(event) # Для других кнопок - стандартная обработка

    def delete_connection(self):
        try:
            # Удаляем из коннекторов
...
class Block(QGraphicsRectItem):
    def __init__(self, title, block_type):
        ...
        # Разные блоки имеют разное количество входов/выходов
...
class BlockEditor(QMainWindow):
    ...
    def execute_pipeline(self):
        ...
        # Здесь должна быть логика выполнения pipeline
    ...
    def find_connected_blocks(self, start_block):
        ...
        while current_block:
            next_block = None
            # Ищем соединения из выходных коннекторов
    ...
    def delete_selected_items(self):
        ...
        self.statusBar().showMessage("Deleted selected connections", 2000)
